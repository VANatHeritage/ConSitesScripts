...
#
# Syntax:
# ConSiteQC_consiteTools(Auto_CS, Old_CS, Old_ID, Cutoff_Value, scratchGDB, QC_Sites)
# ---------------------------------------------------------------------------------------
...
Old_CS = arcpy.GetParameterAsText(1) # input old Conservation Site feature class
Old_ID = arcpy.GetParameterAsText(2) # the unique site ID field in the old CS feature class
Cutoff_Value = arcpy.GetParameter(3) # input a cutoff percentage that will be used to flag features that represent significant boundary growth or reduction(e.g., 10%)
scratchGDB = arcpy.GetParameterAsText(4) # scratch geodatabase for intermediate products
QC_Sites = arcpy.GetParameterAsText(5) # output new Conservation Sites feature class with QC information
...
arcpy.SpatialJoin_analysis(Auto_CS, Old_CS, Join1, "JOIN_ONE_TO_ONE", "KEEP_ALL", fldmap, "INTERSECT", "", "")
...
arcpy.SpatialJoin_analysis(Old_CS, "ssLyr", Join2, "JOIN_ONE_TO_ONE", "KEEP_COMMON", fldmap, "INTERSECT", "", "")
...
# The only reason to make a layer is for a later "Select Layer by Location" step
arcpy.MakeFeatureLayer_management(Old_CS, "oldLyr")
...
arcpy.SelectLayerByLocation_management("SingleLyr", "ARE_IDENTICAL_TO", Old_CS, "", "NEW_SELECTION", "NOT_INVERT")
...
arcpy.Merge_management (fcList, QC_Sites) 
