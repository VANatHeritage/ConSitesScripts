...
def main():
   ...
   # Parse out input PF and ConSite types
   fcList = ParseSiteTypes(in_ProcFeats, in_ConSites, out_GDB)
   ...
   if siteType in ("TCS", "COMBO"):
      if countFeatures(pfTCS) > 0:
         ...
         CreateSBBs(pfTCS, fld_SFID, fld_Rule, fld_Buff, in_nwi5, in_nwi67, in_nwi9, sbb1, scratchGDB)
         
         # Expand SBBs with Cores
         printMsg("Expanding SBBs...")
         ExpandSBBs(in_Cores, sbb1, pfTCS, fld_SFID, sbb2, scratchGDB)
         
         # Create ConSites
         printMsg("Creating Sites...")
         CreateConSites(sbb2, ysn_Expand, pfTCS, fld_SFID, in_ConSites, out_TCS, "TERRESTRIAL", in_Hydro, in_TranSurf, in_Exclude, scratchGDB)
         
         # Review ConSites
         if ysnQC == "Y":
            printMsg("Comparing new sites to old sites for QC...")
            ReviewConSites(out_TCS, csTCS, cutVal, out_TCSqc, fld_SiteID, scratchGDB)
   ...
   if siteType in ("AHZ", "COMBO"):
      if countFeatures(pfAHZ) > 0:
         ...
         # Create SBBs
         printMsg("Creating SBBs...")
         CreateSBBs(pfAHZ, fld_SFID, fld_Rule, fld_Buff, in_nwi5, in_nwi67, in_nwi9, sbb1, scratchGDB)
         
         # Create ConSites
         printMsg("Creating Sites...")
         CreateConSites(sbb2, ysn_Expand, pfAHZ, fld_SFID, in_ConSites, out_AHZ, "AHZ", in_Hydro, in_TranSurf, in_Exclude, scratchGDB)
   ...
   if siteType in ("SCS", "COMBO"):
      if countFeatures(pfSCS) > 0:
         printMsg("Working on Stream Conservation Sites...")
         ...
         MakeNetworkPts_scs(in_hydroNet, in_Catch, pfSCS, scsPts)
         
         # Create SCS lines
         printMsg("Creating SCU lines...")
         CreateLines_scs(scsLines, pfSCS, scsPts, lyrDownTrace, lyrUpTrace)
         ...
         if ysnQC == "Y":
            printMsg("Comparing new sites to old sites for QC...")
            ReviewConSites(out_SCS, csSCS, cutVal, out_SCSqc, fld_SiteID, scratchGDB)
