...
# Summary:
# A collection of functions to process National Wetlands Inventory data to make it more user-friendly for various projects.

# Usage tips:
# NWI data are available for download by state at https://www.fws.gov/wetlands/Data/State-Downloads.html. 
# An NWI code diagram is here: https://www.fws.gov/wetlands/documents/NWI_Wetlands_and_Deepwater_Map_Code_Diagram.pdf
# ---------------------------------------------------------------------------
...
def TabParseNWI(inNWI, outTab):
   '''Given a National Wetlands Inventory (NWI) feature class, creates a table containing one record for each unique code in the ATTRIBUTE field. The codes in the ATTRIBUTE field are then parsed into into easily comprehensible fields, to facilitate processing and mapping. (Adapted from a Model-Builder tool and a script tool.) 
   ...
   dRiv = {'1':'Tidal', 
           ...
           '3':'Upper Perennial',
           '4':'Intermittent'}
   # For dRiv, note that 5: Unknown Perenial is no longer a valid code and has been removed from the dictionary
...
def SubsetNWI(inNWI, inTab, inGDB):
   ...
   Parameters:
   - inNWI: Polygon features representing wetlands, from NWI. 
...
def main():
   ...
   inNWI = r'H:\Backups\GIS_Data_VA\NWI\VA_NWI_2018\VA_NWI_2018.gdb\VA_Wetlands'
   outTab = r'H:\Backups\GIS_Data_VA\NWI\VA_NWI_2018\VA_NWI_2018.gdb\VA_Wetlands_Codes'
   inGDB = r'H:\Backups\GIS_Data_VA\NWI\VA_NWI_2018\NWI_SBB_Subsets.gdb'
   
   # Set up function runs here
   TabParseNWI(inNWI, outTab)
   SbbToNWI(outTab)
   SubsetNWI(inNWI, outTab, inGDB)
